...
public class RobotContainer {
  ...
  //creates commands and instantiates the turret
  ...
  public RobotContainer() {
    //creates new joystick and turret objects
    joy = new Joystick(0);
    turret = new Turret();

    turret.setAngle(0); //sets the turret angle to start at 0

    //defines commands the robot can perform
    teleOp = new TurnToAngle(turret, 0, joy, true); //only manual control command, all others are autonomous
    ...
    turret.setDefaultCommand(teleOp); //Robot is manually controlled by default
    configureButtonBindings(); //Calls the method to assign commands to buttons on the joystick
  }
  ...
  private void configureButtonBindings() {
    //Assigns commands to joystick buttons
    ...
    switchMode.whenPressed(teleOp); //Switches control to teleOp
